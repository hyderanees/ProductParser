...
def product_competitor_job():
    cron_ = CronJobLogRunning.objects.create(cron_job_type=1)
    products = ProductCompetitorsLinks.objects.all().values('product__id').distinct()
    aloj = AdminLogOfJobToShow.objects.filter(product__id__in=products,
                                              created_at__date=datetime.datetime.now().date()).values('product__id')
    products = Products.objects.filter(id__in=products).exclude(id__in=aloj)
    ...
    for prod in products:
        ...
        print(prod.id, 'product id')

        for obj in pcl:
            print(obj.id, 'competitor id')
            response_ = FetchProductDetail(obj)
            response_.iterate_over_obj()
            response_ = response_.response_param
            old_price = ""

            pcsr____ = ProductCompetitorSitesReference.objects.filter(product=obj.product,
                                                                      competitor_link=obj,
                                                                      link=obj.link,
                                                                      is_error=False).last()
            if pcsr____:
                old_price = pcsr____.promotion_price

            if response_:
                old_price_date = None
                if pcsr____:
                    if pcsr____.promotion_price != str(response_['promotion_price']):
                        if response_['stock'] != 'no stock' or response_['stock'] != '' or response_['stock'] != 'N/A':
                            is_price_change = True
                            old_price_date = datetime.datetime.now().date()
                else:
                    if response_['stock'] != 'no stock' or response_['stock'] != '' or response_['stock'] != 'N/A':
                        is_price_change = True
                ...
                pcsr = ProductCompetitorSitesReference.objects.create(product=obj.product,
                                                                      ...
                                                                      is_error=False)
                if old_price_date:
                    pcsr.old_price_date = old_price_date
                    pcsr.save()
        ...
        try:
            prod.cheapest_comp = float(cheapest_price)
            prod.save()
        except Exception as e:
            print (e)
